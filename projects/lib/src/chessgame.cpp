...
void ChessGame::onMoveMade(const Chess::Move& move)
{
    ...
    MoveEvaluation moveEvaluation = sender->evaluation();

    if (sender->side() == Chess::Side::Black)
        moveEvaluation.setScore(-moveEvaluation.score());
    m_scores[m_moves.size()] = moveEvaluation.score();

    m_moves.append(move);
    addPgnMove(move, evalString(moveEvaluation));
...
void ChessGame::setBookOwnerGuardp(bool enabled)
{
...
void ChessGame::emitLastMovePub()
{
    this->emitLastMove();
};
...
void ChessGame::initializePgn()
{
    ...
    {
        m_pgn->setTag("RedTimeControl", m_timeControl[Chess::Side::Red].toString());
