...
"""
Usage: clop-sylvan-cli.py CPU_ID SEED [PARAM_NAME PARAM_VALUE]...
Run sylvan-cli with CLOP_PARAM(s).
...
This script works between CLOP and sylvan-cli. The path to this script,
...
In this script the variables 'sylvan_cli_path', 'engine', 'engine_param_cmd',
...
# Path to the sylvan-cli executable.
# On Windows this should point to sylvan-cli.exe
sylvan_cli_path = 'path_to_sylvan-cli/sylvan-cli.sh'
...
# Additional sylvan-cli options, eg. time control and opening book
...
def main(argv = None):
    ...
    for i in range(1, len(argv), 2):
        ...
        # sylvan-cli's initialization string feature
    ...
    sylvan_args = '-engine %s -engine %s %s' % (fcp, scp, options)
    command = '%s %s' % (sylvan_cli_path, sylvan_args)

    # Run sylvan-cli and wait for it to finish
    ...
    # Convert sylvan-cli's result into W/L/D
